...
public class PostDAOImpl implements PostDAO {
	...
	public int insertPost(PostDTO postDTO) throws SQLException {
		int result = 0;
		sql = "INSERT INTO POSTS(POST_NO,POST_TYPE_NO, USER_NO, POST_TITLE, POST_CONTENTS, POST_VIEW_COUNTS, POST_DATE)"
				+ " VALUES(POST_NO_SEQ.NEXTVAL,?,?,?,?,?,SYSDATE)";
	...
	public int updatePost(PostDTO dto) throws SQLException {
		...
		sql = "UPDATE POSTS SET POST_TITLE=?, POST_CONTENTS=? WHERE USER_NO=? AND POST_NO=?";
		...
		st.setInt(3, dto.getUserNo());
		st.setInt(4, dto.getNo());
	 ...
	 */
	@Override
	public int deletePost(int userNo, int postNo) throws SQLException { // 댓글도 같이 삭제되는지 확인
		int result = 0;
	...
	/**
	 * 댓글 존재 여부 확인 - 댓글부터 지워야 게시글이 지워짐.. - 이거 임시. cascade 수정할까
	 */
	public int deleteCommentCheck(int userNo, int postNo) throws SQLException {
		...
		if (rs.next()) {
			result = rs.getInt(1); // 컬럼의 첫번째 값을 int형 가져옴
		}
	...
	public List<PostDTO> selectUserPost(int userNo) throws SQLException {
		...
		if (userNo == 0  && "".equals(userNo)) { //사용자 있으면 sql구문 추가 & int로 받을지 String으로 받을지
		...
		while(rs.next()) {
			PostDTO postDTO = new PostDTO(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getString(4), rs.getString(5), 
					rs.getInt(6), rs.getString(7)); // 컬럼의 첫번째 값을 int형 가져옴
	...
	public List<PostDTO> selectPost(String userNo) throws SQLException {
		...
		sb.append("SELECT POST_NO, POST_TYPE_NO, USER_NO, POST_TITLE, POST_CONTENTS, POST_VIEW_COUNTS, POST_DATE").append(SPACE);
		sb.append("FROM POSTS");
		if (!"".equals(userNo)) {
			sb.append(SPACE).append("WHERE USER_NO=?");
		}
	...
	/**
	 * 게시판 상위 공간 오늘의 신규 레시피 - 오늘 날짜에 등록된 레시피
	 */
	@Override
