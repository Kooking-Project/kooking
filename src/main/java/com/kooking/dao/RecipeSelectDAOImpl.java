...
public class RecipeSelectDAOImpl implements RecipeSelectDAO{
	...
	public RecipeWrapper search(int postNo) throws Exception{
		...
		try {
			...
			Entry<PostDTO, RecipeDTO> entry = getRecipe(con, postNo);
			...
			rw.setIgredients(getIngredients(con, recipeNo));
			rw.setProcess(getProcesses(con, recipeNo));
	...
	/**
	 * 레시피 번호로 레시피 게시글을 가져오는 메소드
	 * 레시피 DTO와 POST DTO를 조인하여 모두 가져옴.
	 * @param con - 유지할 Connection 정보, null이라면 Connection을 새로 생성함.
	 * @param postNo - 검색할 레시피 번호
	 * @return PostDTO와 RecipeDTO를 합친 Entry
	 * @throws Exception
	 */
	private Entry<PostDTO, RecipeDTO> getRecipe(Connection con, int postNo) throws Exception{
		if(con == null) {
			con = DBTestUtil.getConnection(); // 커넥션이 없다면 새로운 커넥션을 생성
		}
		PreparedStatement ps = null;
		ResultSet rs = null;
		Entry<PostDTO, RecipeDTO> result = null;
		try {
			ps = con.prepareStatement("SELECT POST_NO, POST_TYPE_NO, POST_TITLE, POST_CONTENTS, POST_VIEW_COUNTS, POST_DATE, RECIPES_NO, RECIPES_NAME, RECIPES_CALORIE, RECIPES_COOKING_TIME, RECIPES_NATION, RECIPES_TYPE, RECIPES_LEVEL FROM POSTS INNER JOIN RECIPES USING(POST_NO) WHERE POST_NO=?");
			ps.setInt(1, postNo);
			rs = ps.executeQuery();
	 ...
	 * @throws Exception
	 */
	private List<IngredientDTO> getIngredients(Connection con, int recipeNo) throws Exception{
		if(con == null) {
			con = DBTestUtil.getConnection(); // 커넥션이 없다면 새로운 커넥션을 생성
		}
		PreparedStatement ps = null;
		...
		try {
			ps = con.prepareStatement("SELECT INGREDIENT_NO, RECIPES_NO, INGREDIENT_NAME, INGREDIENT_SEQ, INGREDIENT_CACTY  FROM INGREDIENTS WHERE RECIPES_NO = ? ORDER BY INGREDIENT_SEQ");
	 ...
	 * @throws Exception
	 */
	private List<ProcessDTO> getProcesses(Connection con, int recipeNo) throws Exception{
		if(con == null) {
			con = DBTestUtil.getConnection(); // 커넥션이 없다면 새로운 커넥션을 생성
		}
		PreparedStatement ps = null;
		...
		try {
			ps = con.prepareStatement("SELECT PROCESS_NO, RECIPES_NO, IMAGE_URL, PROCESS_SEQ, PROCESS_DESC, PROCESS_TIP FROM PROCESS WHERE RECIPES_NO=? ORDER BY PROCESS_SEQ");
			ps.setInt(1, recipeNo);
			rs = ps.executeQuery();
	...
	//TODO : 이미지 테이블에서 재료를 참조하는 방향으로 바꾸기
	private List<ImageDTO> getPostImages(Connection con, int postNo){
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(dao.getRecipe(con, 3));
	}
